...
def section_diurnal(doc, results):
    with doc.create(pl.Section('Diurnal variation')):
        ...
        with doc.create(pl.Figure(position='h!')) as pic:
            ...
            pic.add_caption("Intra-day changes in hepatocellular uptake (k_he, top row) and biliary excretion (k_bh, bottom row) of Gadoxetate at visit 1 (left column) and visit 2 (right column). Full lines connect values taken in the same subject at the same day." )
        ...
        with doc.create(pl.Table(position='h!')) as table:
            ...
            with table.create(pl.Tabular('ll'+'c'*(df.shape[1]-1))) as tab:
                ...
                for row in df.index:
                    tab.add_row([row] + list(df.loc[row,:]))
                tab.add_hline()
            table.add_caption("Results of a pairwise comparison testing for differences in liver biomarkers between baseline visit and visit 2. The results are ranked by their p-value, with most significant differences at the top of the list.")
        ...
        with doc.create(pl.Table(position='h!')) as table:
            ...
            with table.create(pl.Tabular('ll'+'c'*(df.shape[1]-1))) as tab:
                ...
                for row in df.index:
                    tab.add_row([row] + list(df.loc[row,:]))
                tab.add_hline()
            table.add_caption("Mean values along with their 95 percent confidence intervals for all liver biomarkers at the baseline visit and visit 2. The last column shows the relative change at visit 2. The results are ranked by their p-value, with most significant differences at the top of the list.")
        ...
        with doc.create(pl.Table(position='h!')) as table:
            ...
            with table.create(pl.Tabular('ll'+'c'*(df.shape[1]-1))) as tab:
                ...
                for row in df.index:
                    tab.add_row([row] + list(df.loc[row,:]))
                tab.add_hline()
            table.add_caption("Results of a pairwise comparison testing for differences in systenic biomarkers between baseline visit and visit 2. The results are ranked by their p-value, with most significant differences at the top of the list.")
        ...
        with doc.create(pl.Table(position='h!')) as table:
            ...
            with table.create(pl.Tabular('ll'+'c'*(df.shape[1]-1))) as tab:
                ...
                for row in df.index:
                    tab.add_row([row] + list(df.loc[row,:]))
                tab.add_hline()
            table.add_caption("Mean values along with their 95 percent confidence intervals for all systemic biomarkers at the baseline visit and visit 2. The last column shows the relative change at visit 2. The results are ranked by their p-value, with most significant differences at the top of the list.")
    ...
    with doc.create(pl.Section('Case notes')):
        ...
        for i, subject in enumerate(df.subject.unique()):
            ...
            with doc.create(pl.Subsection('Subject ' + subj)):
                ...
                # Images
                with doc.create(pl.Figure(position='h!')) as pic:
                    im = os.path.join(results, folder, subj +'_visit1.png')
                    pic.add_image(im, width='5.5in')
                    pic.add_caption("Signal-time curves for subject "+subj+' at visit 1.')

                if not dfr.empty:
                    with doc.create(pl.Figure(position='h!')) as pic:
                        im = os.path.join(results, folder,  subj +'_visit2.png')
                        pic.add_image(im, width='5.5in')
                        pic.add_caption("Signal-time curves for subject "+subj+' at visit 2.')
