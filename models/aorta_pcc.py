...
def model(xdata,
    ...
    Ji = dcmri.inj_step(t, weight, conc, dose, rate, BAT)
...
def plot_fit(xdata, ydata, pars, vars={}, xvalid=None, show=True, save=False, path=None, prefix='', xcheck=None, ycheck=None):
    ...
    lbl = 'Aorta'
    fig.tissue(model, xdata, ydata, pars, xcheck=xcheck, ycheck=ycheck, color=clr, label=lbl, vars=vars, xvalid=xvalid, show=show, save=save, path=path, prefix=prefix)
    BAT = pars[0]
    fig.tissue(model, xdata, ydata, pars, xcheck=xcheck, ycheck=ycheck, win='pass1', xlim=[BAT-20, BAT+160], color=clr, label=lbl, vars=vars, xvalid=xvalid, show=show, save=save, path=path, prefix=prefix)