...
def format(data):
    xvalid = np.concatenate([data['aorta_valid1'], data['aorta_valid2']])
    xdata = np.concatenate([data['time1'], data['time2']])
    ydata = np.concatenate([data['aorta1'], data['aorta2']])
    ...
    pars, vars  = ao.init(xdata, ydata, pars, vars, 1000.0/data['T1aorta3'])
...
def fit(data, path, name):
    ...
    # Fit model to data
    loss = tools.loss(ao.model, xdata, ydata,
    ...
    R1 = [1000.0/data['T1aorta1'], 1000.0/data['T1aorta2'], 1000.0/data['T1aorta3']]
    ycheck = [
    ...
    ao.plot_fit(xdata, ydata, pars, vars=vars, xvalid=xvalid, save=True, show=False, path=path, prefix=name, xcheck=xcheck, ycheck=ycheck)
