    ...
    for subj in rois.keys():
        for visit in rois[subj].keys():
            for tacq in acq_times:
                ...
                fit_subj(rois, pars, subj, visit, resultspath, name, tacq)
    
    print('Calculation time (mins): ', (time.time()-start)/60)


def fit_subj(rois, pars, subj, visit, path, name, tacq=None):
    ...
    if tacq is None:
        ...
        study = visit + '_' + str(tacq).zfill(2)
    tools.to_dmr(os.path.join(path, 'Results'), subj, study, name, pars)
